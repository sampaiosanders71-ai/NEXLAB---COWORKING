(function(){
  if (window.__NEXLAB_V25_17_13_VISUAL__) return;
  window.__NEXLAB_V25_17_13_VISUAL__ = true;

  const VERSION = '25.17.13';
  const BRAND = 'brand-r26';
  const ONBOARDING_KEY = 'nexlab:onboarding:v1:done';
  const CHANGELOG_KEY = 'nexlab:changelog:25.17.13:seen';
  ...
  function warmOfflineCache(){
    ...
    const files = [
      ...
      './changelog.html',
      './publicacao.html',
      `./assets/nexlab-bootstrap-v25-17-13.js?v=${BRAND}`,
      `./assets/index-CKPsxFpP.js?v=${BRAND}`,
      `./assets/index-B1u8eynw.css?v=${BRAND}`,
      `./assets/nexlab-visual-v25-17-13.css?v=${BRAND}`,
      `./assets/nexlab-visual-v25-17-13.js?v=${BRAND}`,
    ...
    caches.open('nexlab-v25-17-13-runtime-r1')
  ...
  function publicationChecklistMarkup(){
    const items = [
      ['Versão e cache', 'Confirmar NEXLAB v' + VERSION + ', manifest, Service Worker, cache brand-r26 e botão “Atualizar app”.'],
  ...
  function copyPublicationChecklist(){
    const text = [
      ...
      '- Versão e cache: manifest, Service Worker, cache brand-r26 e botão Atualizar app.',
  ...
  let nexlabNavLockUntil = 0;
  let nexlabNavTimer = null;
  let nexlabLastPage = '';

  function getActiveModuleLabel(){
    const header = document.querySelector('header.sticky span.font-semibold, header span.font-semibold, header h1')?.textContent || '';
    const h1 = document.querySelector('main h1, main .module-title')?.textContent || '';
    return (header || h1 || document.body.dataset.nexlabPage || 'módulo').trim();
  }

  function showModuleTransition(label){
    let bar = document.querySelector('.nexlab-module-transition');
    if(!bar){
      bar = document.createElement('div');
      bar.className = 'nexlab-module-transition';
      bar.setAttribute('role','status');
      bar.setAttribute('aria-live','polite');
      bar.innerHTML = '<span class="nexlab-module-spinner"></span><span class="nexlab-module-transition-text"></span>';
      document.body.appendChild(bar);
    }
    bar.querySelector('.nexlab-module-transition-text').textContent = `Abrindo ${label || 'módulo'}...`;
    bar.classList.add('is-visible');
    window.clearTimeout(nexlabNavTimer);
    nexlabNavTimer = window.setTimeout(() => hideModuleTransition(true), 1200);
  }

  function hideModuleTransition(force){
    const bar = document.querySelector('.nexlab-module-transition');
    if(!bar) return;
    if(force || Date.now() > nexlabNavLockUntil) bar.classList.remove('is-visible');
  }

  function stabilizeSidebarNavigation(){
    const sidebar = document.querySelector('#mobile-sidebar, aside#mobile-sidebar, aside, [id*="sidebar" i], [class*="sidebar" i]');
    if(!sidebar || sidebar.dataset.nexlabNavStable === 'true') return;
    sidebar.dataset.nexlabNavStable = 'true';
    sidebar.addEventListener('click', event => {
      const target = event.target.closest('button,a');
      if(!target || !sidebar.contains(target)) return;
      const text = (target.textContent || '').replace(/\s+/g,' ').trim();
      if(!text || /Ajuda|Sand|Sair/i.test(text)) return;
      const now = Date.now();
      if(now < nexlabNavLockUntil){
        event.preventDefault();
        event.stopPropagation();
        return;
      }
      nexlabNavLockUntil = now + 700;
      document.body.classList.add('nexlab-module-switching');
      showModuleTransition(text);
      window.setTimeout(() => {
        document.body.classList.remove('nexlab-module-switching');
        hideModuleTransition(true);
      }, 1350);
    }, true);
  }

  function removeStuckSidebarSpinners(){
    const sidebar = document.querySelector('#mobile-sidebar, aside#mobile-sidebar, aside, [id*="sidebar" i], [class*="sidebar" i]');
    if(!sidebar) return;
    ...
    items.forEach(item => {
      ...
      if(!label) return;
      const active = item.getAttribute('aria-current') === 'page' || item.classList.contains('active') || /bg-orange|text-white/.test(item.className || '');
      if(active) return;
      const icons = Array.from(item.querySelectorAll('svg'));
      icons.forEach((svg, idx) => {
        const cls = svg.getAttribute('class') || '';
        const looksLoader = /animate-spin|loader|circle/i.test(cls) || (idx > 0 && svg.getBoundingClientRect && svg.getBoundingClientRect().width <= 22);
        if(looksLoader && idx > 0){
  ...
  function stabilizeModuleBlankState(){
    const page = document.body.dataset.nexlabPage || '';
    if(!page || page === nexlabLastPage){
      removeStuckSidebarSpinners();
      return;
    }
    nexlabLastPage = page;
    window.setTimeout(() => {
      ...
      const hasOnlyLoader = /carregando/i.test(text) && text.length < 80;
      if(hasOnlyLoader){
        const note = document.createElement('div');
        note.className = 'nexlab-module-timeout-note';
        note.innerHTML = '<strong>Carregamento demorando.</strong><span>Atualize os dados ou troque de módulo novamente. O app não precisa ser reiniciado.</span>';
        if(!main.querySelector('.nexlab-module-timeout-note')) main.appendChild(note);
      }
      removeStuckSidebarSpinners();
    }, 6500);
  }
  ...
  function removeModuleHeaderSubtitles(){
    ...
    if(document.body.dataset.nexlabPage === 'marketing'){
      main.querySelectorAll('.nexlab-mkt-hero-copy p, .nexlab-mkt-agenda-head span').forEach(el => {
        el.dataset.nexlabModuleSubtitleHidden = 'true';
      });
    }

    const topBlocks = Array.from(main.querySelectorAll('section, article, div'))
      .filter(el => {
        if(el.closest('form, dialog, table, [role="table"], .nexlab-mkt-agenda, .nexlab-mkt-calendar')) return false;
        const rect = el.getBoundingClientRect ? el.getBoundingClientRect() : {top:9999,height:9999};
        if(rect.top > 360 || rect.height > 260) return false;
        const hasTitle = !!el.querySelector('h1,h2,h3');
        const hasSubtitle = !!el.querySelector('p');
        const text = (el.textContent || '').replace(/\s+/g,' ').trim();
        return hasTitle && hasSubtitle && text.length < 520;
      });

    topBlocks.forEach(block => {
      Array.from(block.querySelectorAll('p')).forEach(p => {
        const text = (p.textContent || '').replace(/\s+/g,' ').trim();
        if(text.length >= 12 && text.length <= 180 && !p.closest('form, dialog, table, [role="table"]')){
          p.dataset.nexlabModuleSubtitleHidden = 'true';
        }
      });
    });
  }
  ...
  function run(){
    ...
    stabilizeSidebarNavigation();
    removeStuckSidebarSpinners();
    stabilizeModuleBlankState();
