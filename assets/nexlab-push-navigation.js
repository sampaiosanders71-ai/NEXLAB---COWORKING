/* NEXLAB Beta 0.26.11 — navegação Push com confirmação explícita, deduplicação e fallback seguro. */
(()=>{
  if(globalThis.__NEXLAB_PUSH_NAVIGATION__?.version==='0.26.11')return;
  ...
  const writeEvidence=(patch)=>{
    ...
    const next={...current,version:VERSION,revision:'beta-0-26-11-device-homologation-server-evidence',userAgent:navigator.userAgent,updatedAt:new Date().toISOString(),...patch};
