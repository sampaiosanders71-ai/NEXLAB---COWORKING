(function(){
  ...
  const CURRENT_VERSION = '26.31.22';
  const CURRENT_RELEASE = 'R56.0.22';
  const CURRENT_REVISION = 'rc17-performance-hardening-r56022';
  const CURRENT_GENERATED_AT = '2026-07-19T00:50:04Z';
  ...
  async function reloadForIdentity(identity, reason){
    ...
    try {
      sessionStorage.setItem('nexlab:last-activated-revision', normalized.revision || normalized.generatedAt || String(Date.now()));
    } catch {}
  ...
  async function check(options = {}){
    ...
    try {
      ...
      if (await reloadForIdentity(activeIdentity, 'active-worker-newer-than-page')) {
        return { ok: true, reloading: true, ...state };
      }
  ...
  async function applyUpdate(){
    ...
    try {
      ...
      if (!waitingWorker || !isNewer(identity)) {
        const activeIdentity = await workerIdentity(registration.active || navigator.serviceWorker.controller);
        if (await reloadForIdentity(activeIdentity, 'active-after-manual-update')) {
          return { ok: true, action: 'reload-active' };
        }
      ...
      dispatch('nexlab:update-state', { ...state });
      return { ok: true, action: 'activation-requested', identity };
    } catch (error) {
      applying = false;
      state.status = 'error';
      state.error = String(error?.message || error);
  ...
  function start(){
    ...
    navigator.serviceWorker.addEventListener('controllerchange', async () => {
      ...
      window.setTimeout(() => {
        reloadForIdentity(identity, applying ? 'manual-controller-change-fallback' : 'controller-change-fallback');
      }, 1200);
    });
    ...
    navigator.serviceWorker.addEventListener('message', (event) => {
      ...
      dispatch('nexlab:update-state', { ...state });
      window.setTimeout(() => {
        reloadForIdentity(event.data, applying ? 'manual-worker-activated-fallback' : 'worker-activated-fallback');
      }, 1200);
    });
