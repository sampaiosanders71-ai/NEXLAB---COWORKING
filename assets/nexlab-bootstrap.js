(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.11',release:'Beta',revision:'beta-0-26-11-device-homologation-server-evidence',assetRevision:'app-beta-0-26-11-device-homologation-server-evidence',cacheName:'nexlab-beta-0-26-11-device-homologation-server-evidence',generatedAt:'2026-07-19T19:39:27Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.11';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.11';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.11', JSON.stringify(performanceState));
    } catch {}
  ...
  function collectStaticPerformanceMetrics(){
    try {
      ...
      const initialPattern = /assets\/(?:index-beta-0-26-11|nexlab-vendor-beta-0-26-11|nexlab-app-shared-beta-0-26-11|nexlab-bootstrap|nexlab-realtime-core-beta-0-26-11|nexlab-realtime-hub-beta-0-26-11|nexlab-update-manager|nexlab-visual|nexlab-vapid-rotation)\.js/i;
      const featurePattern = /assets\/nexlab-feature-modules-beta-0-26-11\.js/i;
  ...
  try {
    if ('PerformanceObserver' in window) {
      ...
      try {
        new PerformanceObserver((list) => {
          if (list.getEntries().some((entry) => /assets\/nexlab-feature-modules-beta-0-26-11\.js/i.test(entry.name))) {
