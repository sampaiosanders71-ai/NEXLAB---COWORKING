(function(){
  ...
  function classifySupabaseRequest(target, method){
    ...
    const readOnly = READ_METHOD_RE.test(normalizedMethod) || Boolean(rpcName && READ_ONLY_RPC_NAME_RE.test(rpcName));
    const background = Boolean(rpcName && BACKGROUND_RPC_NAME_RE.test(rpcName));
    return Object.freeze({ rpcName, readOnly, background, mutating: MUTATING_METHOD_RE.test(normalizedMethod) && !readOnly && !background });
  }
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v26.31.22';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v26.31.22';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v26.31.22', JSON.stringify(performanceState));
    } catch {}
  ...
  function collectStaticPerformanceMetrics(){
    try {
      ...
      const initialPattern = /assets\/(?:index-R56v263122rc17|nexlab-vendor-r56022rc17|nexlab-app-shared-r56022rc17|nexlab-bootstrap|nexlab-realtime-core-r56022rc17|nexlab-realtime-hub-r56022rc17|nexlab-update-manager|nexlab-visual|nexlab-vapid-rotation)\.js/i;
      const featurePattern = /assets\/nexlab-feature-modules-r56022rc17\.js/i;
  ...
  try {
    if ('PerformanceObserver' in window) {
      ...
      try {
        new PerformanceObserver((list) => {
          if (list.getEntries().some((entry) => /assets\/nexlab-feature-modules-r56022rc17\.js/i.test(entry.name))) {
