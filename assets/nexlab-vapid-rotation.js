(function(){
  ...
  window.__NEXLAB_VAPID_ROTATION_R55620__={version:'26.30.45',status:'idle'};
  ...
  async function rotate(){
    ...
    localStorage.setItem(MARKER,'26.30.45');
    state.status=same?'refreshed':'rotated';state.endpoint=subscription.endpoint;state.completedAt=new Date().toISOString();
    window.dispatchEvent(new CustomEvent('nexlab:vapid-rotated',{detail:{status:state.status,version:'26.30.45'}}));
  }
  ...
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&localStorage.getItem(MARKER)!=='26.30.45')start()});
})();
