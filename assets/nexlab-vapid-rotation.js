(function(){
  'use strict';
  if(window.__NEXLAB_VAPID_ROTATION_R56020__) return;
  window.__NEXLAB_VAPID_ROTATION_R56020__={version:'26.31.22',status:'idle'};
  ...
  function markerValue(subscription){return `26.31.22:${subscription?.endpoint||''}`}
  ...
  async function rotate(){
    const state=window.__NEXLAB_VAPID_ROTATION_R56020__;state.status='checking';
    ...
    window.dispatchEvent(new CustomEvent('nexlab:vapid-rotated',{detail:{status:state.status,version:'26.31.22'}}));
  }
  const start=()=>rotate().catch(error=>{window.__NEXLAB_VAPID_ROTATION_R56020__.status='error';window.__NEXLAB_VAPID_ROTATION_R56020__.error=String(error?.message||error)});
  if(document.readyState==='complete')setTimeout(start,1200);else window.addEventListener('load',()=>setTimeout(start,1200),{once:true});
  document.addEventListener('visibilitychange',()=>{const state=window.__NEXLAB_VAPID_ROTATION_R56020__;if(document.visibilityState==='visible'&&['error','no-session'].includes(state.status))start()});
})();
