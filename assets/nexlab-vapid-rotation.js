(function(){
  ...
  window.__NEXLAB_VAPID_ROTATION_R5561__={version:'26.30.19',status:'idle'};
  ...
  async function rotate(){
    ...
    localStorage.setItem(MARKER,'26.30.19');
    state.status=same?'refreshed':'rotated';state.endpoint=subscription.endpoint;state.completedAt=new Date().toISOString();
    window.dispatchEvent(new CustomEvent('nexlab:vapid-rotated',{detail:{status:state.status,version:'26.30.19'}}));
  }
  ...
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&localStorage.getItem(MARKER)!=='26.30.19')start()});
})();
