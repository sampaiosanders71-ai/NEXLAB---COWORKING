(function(){
  'use strict';
  if(window.__NEXLAB_VAPID_ROTATION_R55615__) return;
  window.__NEXLAB_VAPID_ROTATION_R55615__={version:'26.30.28',status:'idle'};
  ...
  async function rotate(){
    const state=window.__NEXLAB_VAPID_ROTATION_R55615__;state.status='checking';
    ...
    localStorage.setItem(MARKER,'26.30.28');
    state.status=same?'refreshed':'rotated';state.endpoint=subscription.endpoint;state.completedAt=new Date().toISOString();
    window.dispatchEvent(new CustomEvent('nexlab:vapid-rotated',{detail:{status:state.status,version:'26.30.28'}}));
  }
  const start=()=>rotate().catch(error=>{window.__NEXLAB_VAPID_ROTATION_R55615__.status='error';window.__NEXLAB_VAPID_ROTATION_R55615__.error=String(error?.message||error)});
  if(document.readyState==='complete')setTimeout(start,1200);else window.addEventListener('load',()=>setTimeout(start,1200),{once:true});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&localStorage.getItem(MARKER)!=='26.30.28')start()});
})();
