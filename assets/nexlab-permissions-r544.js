(function(){
  ...
  function permissionPassword(){return document.querySelector('.nexlab-permission-password-inline input')?.value?.trim()||''}
  ...
  async function loadMatrix(force=false){
    ...
    catch(error){console.error('Permissões R54.5:',error);toast(error.message||'Não foi possível carregar a matriz de permissões.','err')}
  ...
  async function subscribeRealtime(){
    ...
    try{
      ...
      state.channel=client.channel('nexlab-permissions-r544')
