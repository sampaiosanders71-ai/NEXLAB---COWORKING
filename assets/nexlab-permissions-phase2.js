(function(){
  ...
  function permissionPage(){
    const content=document.querySelector('.permission-content-card');
    if(!content)return null;
    return document.querySelector('.permission-matrix-page')||content.closest('main')||content.parentElement;
  }
  function isPermissionScreen(){return Boolean(document.querySelector('.permission-content-card')&&document.querySelector('.permission-sidebar-card'))}
  function ensureSensitivePasswordField(){
    if(!isPermissionScreen())return null;
    const sidebar=document.querySelector('.permission-sidebar-card');if(!sidebar)return null;
    let panel=sidebar.querySelector('.nexlab-sensitive-password-inline');
    if(!panel){
      panel=document.createElement('section');panel.className='nexlab-sensitive-password-inline';
      panel.innerHTML=`<div class="nexlab-sensitive-password-inline-head"><span aria-hidden="true">🔐</span><div><strong>Senha administrativa</strong><small>Obrigatória somente para ativar “Exportar dados pessoais identificáveis”.</small></div></div><label for="nexlab-sensitive-password-input">Digite a senha</label><div class="nexlab-sensitive-password-input-wrap"><input id="nexlab-sensitive-password-input" type="password" autocomplete="current-password" placeholder="Senha administrativa"><button type="button" class="nexlab-sensitive-password-toggle" aria-label="Mostrar senha">Mostrar</button></div><div class="nexlab-sensitive-password-inline-error" aria-live="polite"></div>`;
      const textarea=sidebar.querySelector('textarea');
      const saveButton=[...sidebar.querySelectorAll('button')].find(button=>/salvar/i.test(button.textContent||''));
      if(saveButton)saveButton.insertAdjacentElement('beforebegin',panel);else if(textarea)textarea.insertAdjacentElement('afterend',panel);else sidebar.appendChild(panel);
    ...
    syncSensitivePasswordField();return panel;
  }
  function sensitivePassword(){return ensureSensitivePasswordField()?.querySelector('input')?.value||''}
  function clearSensitivePassword(){const panel=document.querySelector('.nexlab-sensitive-password-inline');const input=panel?.querySelector('input');if(input)input.value='';if(panel){panel.dataset.filled='false';panel.querySelector('.nexlab-sensitive-password-inline-error').textContent=''}}
  function showSensitivePasswordError(message){const panel=ensureSensitivePasswordField();if(!panel)return;panel.dataset.required='true';panel.querySelector('.nexlab-sensitive-password-inline-error').textContent=message;const input=panel.querySelector('input');input.focus();input.scrollIntoView({behavior:'smooth',block:'center'})}
  ...
  function syncSensitivePasswordField(){const panel=document.querySelector('.nexlab-sensitive-password-inline');if(!panel)return;const pending=sensitiveGrantPending();panel.dataset.required=pending?'true':'false';const hint=panel.querySelector('small');if(hint)hint.textContent=pending?'Digite a senha antes de salvar esta concessão.':'Obrigatória somente para ativar “Exportar dados pessoais identificáveis”.'}
  ...
  async function saveRole(){
    ...
    let password=null;if(changes.action_sensitive_export===true){password=sensitivePassword();if(!password){showSensitivePasswordError('Digite a senha administrativa antes de salvar.');return toast('Digite a senha administrativa.','err')}}
  ...
  async function saveUser(){
    ...
    let password=null;if(changes.action_sensitive_export==='allow'){password=sensitivePassword();if(!password){showSensitivePasswordError('Digite a senha administrativa antes de salvar.');return toast('Digite a senha administrativa.','err')}}
  ...
  async function handleAction(event){
    const button=event.target.closest?.('.permission-primary-button,.permission-secondary-button');if(!button||!isPermissionScreen())return;
  ...
  function observe(){
    state.observer=new MutationObserver(()=>{if(isPermissionScreen()){renderStatus();annotateDependencies();ensureSensitivePasswordField();if(!state.matrix)loadMatrix()} });state.observer.observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('click',handleAction,true);document.addEventListener('change',event=>{if(isPermissionScreen()){enforceDependencies(event.target);setTimeout(()=>{syncSensitivePasswordField();ensureSensitivePasswordField()},0)}},true);
  }
