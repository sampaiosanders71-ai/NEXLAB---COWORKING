const APP_VERSION="26.30.7";
const APP_REVISION="pendencias-hardfix-r5543";
const CACHE_NAME="nexlab-v26-30-7-shell-r5543";
const RUNTIME_CACHE="nexlab-v26-30-7-runtime-r5543";
const IMAGE_CACHE="nexlab-v26-30-7-images-r5543";
const ASSET_VERSION="app-r26307-pendencias-hardfix-r5543";
const SHELL_FILES=[
  "./","./index.html","./offline.html","./404.html","./homologacao.html",
  "./RELATORIO_NEXLAB_V26_30_7.html","./prontidao.html","./release.json",
  `./manifest.webmanifest?v=${ASSET_VERSION}`,
  `./assets/index-R55v26307.js?v=${ASSET_VERSION}`,
  ...
  `./assets/nexlab-permissions-r550.js?v=disable-legacy-r5543`,
  `./assets/nexlab-route-integrity-r5543.js?v=${ASSET_VERSION}`,
...
function injectOverlays(html){
  let value=html
    ...
    .replace(/<script[^>]+nexlab-permissions-r550\.js[^>]*><\/script>/gi,'')
    .replace(/<script[^>]+nexlab-route-integrity-r5543\.js[^>]*><\/script>/gi,'')
    .replace(/<script[^>]+nexlab-permissions-r5543\.js[^>]*><\/script>/gi,'')
    .replace(/<link[^>]+nexlab-mural-sync-r543\.css[^>]*>/gi,'')
    .replace(/<script[^>]+nexlab-mural-sync-r543\.js[^>]*><\/script>/gi,'');
  const permissionScripts=`<script src="./assets/nexlab-permissions-r550.js?v=disable-legacy-r5543"></script><script src="./assets/nexlab-route-integrity-r5543.js?v=${ASSET_VERSION}"></script><script defer src="./assets/nexlab-permissions-r5543.js?v=${ASSET_VERSION}"></script>`;
  value=value.replace(/<head([^>]*)>/i,`<head$1>${permissionScripts}`);
...
async function networkFirstAsset(request){
  try{
    ...
    const cache=await caches.open(RUNTIME_CACHE);cache.put(request,response.clone()).catch(()=>{});return response;
  }catch{
    return await caches.match(request)||new Response('',{status:503,statusText:'Asset unavailable'});
  }
}
